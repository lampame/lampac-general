...
namespace LMG.StreamData.Controllers
{
    public class Controller : BaseOnlineController
    {
        ...
        async public Task<ActionResult> Index(long id, string imdb_id, long kinopoisk_id, string title, string original_title, string original_language, int year, string source, int serial, string account_email, string t, int s = -1, int e = -1, bool play = false, bool rjson = false, string href = null, bool checksearch = false)
        {
            ...
            if (checksearch)
            {
                if (!IsCheckOnlineSearchEnabled())
                    return OnError("lmg_streamdata", refresh_proxy: true);
        ...
        private async Task<ActionResult> HandleSerial(StreamDataInvoke invoke, OnlinesSettings init, long tmdbId, string title, string originalTitle, int s, int e, string t, bool rjson)
        {
            ...
            if (s <= 0)
            {
                ...
                {
                    string seasonLink = $"{host}/lite/lmg_streamdata?id={tmdbId}&serial=1&s={season}&title={HttpUtility.UrlEncode(displayTitle)}&original_title={HttpUtility.UrlEncode(originalTitle)}";
            ...
            {
                string voiceLink = $"{host}/lite/lmg_streamdata?id={tmdbId}&serial=1&s={s}&t={i}&title={HttpUtility.UrlEncode(displayTitle)}&original_title={HttpUtility.UrlEncode(originalTitle)}";
            ...
            foreach (var epNum in episodeNumbers)
            {
                ...
                string callUrl = $"{host}/lite/lmg_streamdata?id={tmdbId}&serial=1&s={s}&e={epNum}&play=true&t={selectedSource}&title={HttpUtility.UrlEncode(displayTitle)}&original_title={HttpUtility.UrlEncode(originalTitle)}";
